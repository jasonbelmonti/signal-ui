...
export function createPixelCubePathAnimationProfile({ count, cycleMs, seed, }) {
    const random = createPrng(seed);
    const adjustedCycleMs = Math.max(minimumCycleMs, Math.round(cycleMs + (random() - 0.5) * cycleVarianceMs * 2));
    ...
    const normalizedDurations = rawDurations.map((duration) => (duration / totalDuration) * adjustedCycleMs);
...
export function getPixelCubePathAnimationSeed(value) {
    let hash = 2166136261;
