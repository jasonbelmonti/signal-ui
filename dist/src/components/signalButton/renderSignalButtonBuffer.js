...
import { blendChannels, clamp, toneAccentChannels } from "./utils.js";
export function renderSignalButtonBuffer({ ctx, cols, cooldownPercent, disabled = false, edgeWidthPx, fillPercent, pulseBurst, rewardChannels, rows, tone, timeMs, wakePercent, }) {
    ...
    for (let y = 0; y < rows; y += 1) {
        for (let x = 0; x < cols; x += 1) {
            ...
            const pixelAccent = outcomeBlend > 0 ? blendChannels(baseAccent, rewardChannels, outcomeBlend) : surfaceAccent;
            if (distance > 0.25) {
                ...
                const dropoutThreshold = clamp(dropoutBase - edgeGlow * 0.045 - wakeProgress * 0.06, 0.005, 0.14);
            ...
            if (distance > -edgeCells) {
                ...
                if (signalNoise > activeThreshold) {
                    ...
                    const lift = 0.08 + edgeProximity * 0.22 + bodyBlend * 0.08 + (stableNoise > 0.82 ? 0.14 : 0);
                ...
                if (distance > -1.25 && signalNoise > ghostThreshold) {
                    ...
                    drawPixel(ctx, x, y, pixelAccent, 0.72 + bodyBlend * 0.12, 0.04 + edgeProximity * 0.06, alpha);
                }
            }
        }
    }
...
function drawFrontierBloom(ctx, front, rows, edgeCells, accent, disabled) {
...
function getWakeProgress(wakeLevel, wakeSeed) {
    const start = wakeSeed * 0.82;
    return clamp((wakeLevel - start) / 0.18, 0, 1);
}
function getPulseOutcomeBlend(x, y, cols, rows, burstLevel, cooldownLevel) {
    if (burstLevel <= 0.01) {
...
function drawPulseRings(ctx, cols, rows, accent, rewardChannels, burstLevel, disabled) {
    const centerX = cols / 2;
    const centerY = rows / 2;
    const baseAlpha = disabled ? 0.05 : 0.12 + burstLevel * 0.26;
    const darkAlpha = disabled ? 0.05 : 0.16 + burstLevel * 0.18;
    const ringDelays = [0, 0.11, 0.22, 0.33];
    for (let y = 0; y < rows; y += 1) {
        for (let x = 0; x < cols; x += 1) {
            ...
            for (const delay of ringDelays) {
                ...
                const troughMask = clamp(1 - troughDistance / (ringThickness * 0.92), 0, 1) * (radial <= ringRadius ? 1 : 0);
                darkEnergy += troughMask * ringOpacity * 0.78;
            }
            brightEnergy *= centerFade;
            darkEnergy *= centerFade;
            if (darkEnergy > 0.04) {
                drawPixel(ctx, x, y, accent, 0.42, -0.52, clamp(darkAlpha + darkEnergy * 0.24, 0, 0.66));
            }
            if (brightEnergy > 0.04) {
                const ringAccent = blendChannels(accent, rewardChannels, clamp(0.58 + brightEnergy * 0.34, 0, 1));
                drawPixel(ctx, x, y, ringAccent, 1.02, 0.22 + brightEnergy * 0.32, clamp(baseAlpha + brightEnergy * 0.22, 0, 1));
            }
        }
    }
}
function getPulseRadial(x, y, cols, rows, centerX = cols / 2, centerY = rows / 2) {
